...
export default class HTML extends PureComponent {
  ...
  generateDefaultStyles(baseFontStyle = this.props.baseFontStyle) {
    this.setState({
      defaultBlockStyles: generateDefaultBlockStyles(baseFontStyle.fontSize || 14),
      defaultTextStyles: generateDefaultTextStyles(baseFontStyle.fontSize || 14)
    })
  }
      ...
      ? RNElements.map((element, index) => {
          ...
          if (this.renderers[tagName]) {
            const customRenderer =
              typeof this.renderers[tagName] === 'function' ? this.renderers[tagName] : this.renderers[tagName].renderer
          ...
          const style = [
            !tagsStyles || !tagsStyles[tagName]
              ? (Wrapper === Text ? this.state.defaultTextStyles : this.state.defaultBlockStyles)[tagName]
              : undefined,
